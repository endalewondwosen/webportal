...
<div>
    <div class="pagetitle">
        <h1>Fresh Students List</h1>
        <nav>
            <ol class="breadcrumb">
                <li class="breadcrumb-item">
                    <router-link to="/">Home</router-link>
                </li>
                <li class="breadcrumb-item">Fresh Students</li>
                                ...
                                <table class="table align-items-center table-flush ">
                                    <thead class="table-light">
                                        <tr>
                                            ...
                                            <th scope="col" data-sortable=""><a href="#" class="dataTable-sorter">Campus</a></th>
                                            ...
                                            <th scope="col" data-sortable=""><a href="#" class="dataTable-sorter">University Password</a></th>
                                            <th scope="col" data-sortable=""><a href="#" class="dataTable-sorter">Registration Number </a></th>
                                            <th scope="col" data-sortable=""><a href="#" class="dataTable-sorter">Block Number</a></th>
                                            <th scope="col" data-sortable=""><a href="#" class="dataTable-sorter">Dorm Number</a></th>
                                            <th scope="col" data-sortable=""><a href="#" class="dataTable-sorter">Section</a></th>
                                    ...
                                    <tbody>
                                        <tr v-for="(student,index) in freshStudents" :key="student.id">
                                            <th scope="row">{{ index+1 }}</th>
                                            <td>{{ student.first_name }}</td>
                                            <td>{{ student.lastname_name }}</td>
                                            ...
                                            <td>{{ student.dorm_number }}</td>
                                             <td>{{student.section }}</td>
                                            <td>{{ student.year.name }}</td>
                                            <td>{{ student.created_at }}</td>
                                            <td>
                                                <router-link :to="{name:'EditFreshStudent',params:{id:student.id}}" class="btn btn-primary"><i class="ri-edit-2-fill"></i></router-link>
                                                <button class="btn btn-danger" @click.prevent="deleteFreshStudent(student.id)"><i class="bi bi-trash-fill"></i></button>
                                            </td>
                                        </tr>
...
export default {
    ...
    methods: {
        allFreshStudents() {
            this.axios.get('/api/user/fresh_students')
                .then(({
                    data
                }) => {
                    this.freshStudents = data
                }).catch()
        },
