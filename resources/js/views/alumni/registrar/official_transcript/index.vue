                                ...
                                <table class="table align-items-center table-flush ">
                                    <thead class="table-light">
                                        <tr>
                                            ...
                                            <th scope="col" data-sortable=""><a href="#" class="dataTable-sorter">ID NO </a></th>
                                    ...
                                    <tbody>
                                        <tr v-for="(official,index) in applicants" :key="official.id">
                                            ...
                                            <td> {{ official.year.name }} </td>
                                            <!-- <td><a :href="official.bank_receipt">{{ official.bank_receipt }}</a></td> -->
                                            <td>{{ official.university_id }}

                                            </td>
                                              <td >
                                                <span v-if="official.status=='0'" class="badge rounded-pill  bg-warning text-dark">Pending...</span>
                                                <span  v-else-if="official.status=='1'" class="badge rounded-pill bg-success">Approved</span>
                                                 <span v-else class=" badge rounded-pill bg-danger ">Rejected</span>
                                            </td>
                                              
                                            <td>
                                                 <button v-if="official.status=='0'||official.status=='2'" class="btn btn-primary rounded-pill " @click.prevent="approveApplicant(official.id)">Approve</button>
                                                 <button  v-if="official.status=='0'||official.status=='1'" class="btn btn-danger rounded-pill " @click.prevent="rejectAplicant(official.id)"  >Reject</button>
                                                  
                                                <router-link  :to="{name:'ApplicantOfficailTranscriptDetail',params:{id:official.id}}" class="btn btn-info rounded-pill"><i class="ri-edit-2-fill"></i> details</router-link>
...
export default {
    ...
    methods: {
        allApplicants() {
            this.axios.get('/api/user/all_official_applicants')
                .then(({
                    data
                }) => {
        ...
        approveApplicant(id){
          Swal.fire({
                ...
                if (result.isConfirmed) {
                    this.axios.get('/api/user/approveAplicant/' + id)
                        .then(() => {
                        this.allApplicants();
         ...
         rejectAplicant(id){
          Swal.fire({
                ...
                if (result.isConfirmed) {
                    this.axios.get('/api/user/rejectAplicant/' + id)
                        .then(() => {
                        this.allApplicants();
                            //Notification.success(); 
                            Toast.fire({
