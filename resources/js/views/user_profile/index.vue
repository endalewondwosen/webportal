<template>
<div>
    <div class="pagetitle">
        <h1>Profile</h1>
        <nav>
            <ol class="breadcrumb">
                <li class="breadcrumb-item">
                    <router-link to="/home">Home</router-link>
                </li>
                <li class="breadcrumb-item">User</li>
                <li class="breadcrumb-item active">Profile</li>
            </ol>
        </nav>
    </div>
    <!-- End Page Title -->

    <section class="section profile">
        <div class="row">
            <div class="col-xl-4">
                <div class="card">
                    <div class="card-body profile-card pt-4 d-flex flex-column align-items-center">
                        <img :src="`backend/assets/img/profile-img.jpg`" alt="Profile" class="rounded-circle" />
                        <h2>{{ loggedUser.name }}</h2>
                        <h3>{{ loggedUser.role }}</h3>
                        <div class="social-links mt-2">
                            <a href="#" class="twitter"><i class="bi bi-twitter"></i></a>
                            <a href="#" class="facebook"><i class="bi bi-facebook"></i></a>
                            <a href="#" class="instagram"><i class="bi bi-instagram"></i></a>
                            <a href="#" class="linkedin"><i class="bi bi-linkedin"></i></a>
                        </div>
                    </div>
                </div>
            </div>

            <div class="col-xl-8">
                <div class="card">
                    <div class="card-body pt-3">
                        <!-- Bordered Tabs -->
                        <ul class="nav nav-tabs nav-tabs-bordered">
                            <li class="nav-item">
                                <button class="nav-link active" data-bs-toggle="tab" data-bs-target="#profile-overview">
                                    Overview
                                </button>
                            </li>

                            <li class="nav-item">
                                <button class="nav-link" data-bs-toggle="tab" data-bs-target="#profile-edit">
                                    Edit Profile
                                </button>
                            </li>

                            <li class="nav-item">
                                <button class="nav-link" data-bs-toggle="tab" data-bs-target="#profile-change-password">
                                    Change Password
                                </button>
                            </li>
                        </ul>
                        <div class="tab-content pt-2">
                            <div class="tab-pane fade show active profile-overview" id="profile-overview">

                                <h5 class="card-title">Profile Details</h5>

                                <div class="row">
                                    <div class="col-lg-3 col-md-4 label">
                                        Full Name
                                    </div>
                                    <div class="col-lg-9 col-md-8">
                                        {{loggedUser.name}} {{loggedUser.father_name}} {{loggedUser.gfather_name}}
                                    </div>
                                </div>

                                <div class="row">
                                    <div class="col-lg-3 col-md-4 label">
                                        Company
                                    </div>
                                    <div class="col-lg-9 col-md-8">
                                        {{loggedUser.company}}
                                    </div>
                                </div>

                                <div class="row">
                                    <div class="col-lg-3 col-md-4 label">
                                        Job
                                    </div>
                                    <div class="col-lg-9 col-md-8">
                                        {{loggedUser.role}}
                                    </div>
                                </div>

                                <div class="row">
                                    <div class="col-lg-3 col-md-4 label">
                                        Country
                                    </div>
                                    <div class="col-lg-9 col-md-8">{{loggedUser.country}}</div>
                                </div>

                                <div class="row">
                                    <div class="col-lg-3 col-md-4 label">
                                        Address
                                    </div>
                                    <div class="col-lg-9 col-md-8">
                                        {{loggedUser.address}}
                                    </div>
                                </div>

                                <div class="row">
                                    <div class="col-lg-3 col-md-4 label">
                                        Phone
                                    </div>
                                    <div class="col-lg-9 col-md-8">
                                        {{loggedUser.phone}}
                                    </div>
                                </div>

                                <div class="row">
                                    <div class="col-lg-3 col-md-4 label">
                                        Email
                                    </div>
                                    <div class="col-lg-9 col-md-8">
                                        {{loggedUser.email}}
                                    </div>
                                </div>
                            </div>

                            <div class="tab-pane fade profile-edit pt-3" id="profile-edit">
                                <!-- Profile Edit Form -->
                                <form @submit.prevent="updateProfile">
                                    <div class="row mb-3">
                                        <label for="profileImage" class="col-md-4 col-lg-3 col-form-label">Profile Image</label>
                                        <div class="col-md-8 col-lg-9">

                                            <img :src="loggedUser.profile_image" alt="Profile Image" />

                                            <div class="pt-2">
                                                <input type="file" class="form-control">
                                                <a href="#" class="btn btn-danger btn-sm" title="Remove my profile image"><i class="bi bi-trash"></i></a>
                                            </div>
                                        </div>
                                    </div>

                                    <div class="row mb-3">
                                        <label for="fullName" class="col-md-4 col-lg-3 col-form-label">First Name</label>
                                        <div class="col-md-8 col-lg-9">
                                            <input type="text" class="form-control" v-model="form.name" />
                                            <div class="text-danger" v-if="errors.name">
                                                *{{ errors.name[0] }}
                                            </div>
                                        </div>

                                    </div>
                                    <div class="row mb-3">
                                        <label for="fullName" class="col-md-4 col-lg-3 col-form-label">Father Name</label>
                                        <div class="col-md-8 col-lg-9">
                                            <input type="text" class="form-control" v-model="form.father_name" placeholder="Father Name" />
                                            <div class="text-danger" v-if="errors.father_name">
                                                *{{ errors.father_name[0] }}
                                            </div>
                                        </div>
                                    </div>
                                    <div class="row mb-3">
                                        <label for="fullName" class="col-md-4 col-lg-3 col-form-label">Grand Father Name</label>
                                        <div class="col-md-8 col-lg-9">
                                            <input type="text" class="form-control" v-model="form.gfather_name" placeholder="Grand Father Name" />
                                            <div class="text-danger" v-if="errors.gfather_name">
                                                *{{ errors.gfather_name[0] }}
                                            </div>
                                        </div>
                                    </div>

                                    <div class="row mb-3">
                                        <label for="company" class="col-md-4 col-lg-3 col-form-label">Company</label>
                                        <div class="col-md-8 col-lg-9">
                                            <input type="text" class="form-control" v-model="form.company" placeholder="Company Name" />
                                            <div class="text-danger" v-if="errors.company">
                                                *{{ errors.company[0] }}
                                            </div>
                                        </div>
                                    </div>

                                    <div class="row mb-3">
                                        <label for="Country" class="col-md-4 col-lg-3 col-form-label">Country</label>
                                        <div class="col-md-8 col-lg-9">
                                            <input type="text" class="form-control" v-model="form.country" placeholder="Country Name" />
                                            <div class="text-danger" v-if="errors.country">
                                                *{{ errors.country[0] }}
                                            </div>
                                        </div>
                                    </div>

                                    <div class="row mb-3">
                                        <label for="Address" class="col-md-4 col-lg-3 col-form-label">Address</label>
                                        <div class="col-md-8 col-lg-9">
                                            <input type="text" class="form-control" v-model="form.address" placeholder="Address" />
                                            <div class="text-danger" v-if="errors.address">
                                                *{{ errors.address[0] }}
                                            </div>
                                        </div>
                                    </div>

                                    <div class="row mb-3">
                                        <label for="Phone" class="col-md-4 col-lg-3 col-form-label">Phone Number</label>
                                        <div class="col-md-8 col-lg-9">
                                            <input type="text" class="form-control" v-model="form.phone" placeholder="Phone Number" />
                                            <div class="text-danger" v-if="errors.phone">
                                                *{{ errors.phone[0] }}
                                            </div>
                                        </div>
                                    </div>
                                    <div class="row mb-3">
                                        <label for="Phone" class="col-md-4 col-lg-3 col-form-label">Email</label>
                                        <div class="col-md-8 col-lg-9">
                                            <input type="text" class="form-control" v-model="form.email" placeholder="Email" />
                                            <div class="text-danger" v-if="errors.email">
                                                *{{ errors.email[0] }}
                                            </div>
                                        </div>
                                    </div>

                                    <div class="text-center">
                                        <button type="submit" class="btn btn-primary">
                                            Save Changes
                                        </button>
                                    </div>
                                </form>
                                <!-- End Profile Edit Form -->
                            </div>

                            <div class="tab-pane fade pt-3" id="profile-change-password">
                                <!-- Change Password Form -->
                                <form @submit.prevent="passwordUpdate">

                                    <div class="row mb-3">
                                        <label for="currentPassword" class="col-md-4 col-lg-3 col-form-label">Current Password</label>
                                        <div class="col-md-8 col-lg-9">
                                            <input  type="password" class="form-control" v-model="pass.current_password" placeholder="Current Password"  />
                                            <div class="text-danger" v-if="passerrors.current_password">
                                                *{{ passerrors.current_password[0] }}
                                            </div>
                                            <div class="text-danger" v-if="currError">
                                                *{{ currError }}
                                            </div>
                                        </div>
                                    </div>

                                    <div class="row mb-3">
                                        <label for="newPassword" class="col-md-4 col-lg-3 col-form-label">New Password</label>
                                        <div class="col-md-8 col-lg-9">
                                            <input  type="password" class="form-control" v-model="pass.new_password" placeholder="New Password" />
                                            <div class="text-danger" v-if="passerrors.new_password">
                                                *{{ passerrors.new_password[0] }}
                                            </div>
                                        </div>
                                    </div>

                                    <div class="row mb-3">
                                        <label for="renewPassword" class="col-md-4 col-lg-3 col-form-label">Re-enter New Password</label>
                                        <div class="col-md-8 col-lg-9">
                                            <input  type="password" class="form-control" v-model="pass.new_password_confirmation" placeholder="Re-enter New Password" />
                                            <div class="text-danger" v-if="passerrors.new_password_confirmation"> *{{ passerrors.new_password_confirmation[0] }}</div>
                                        </div>
                                    </div>

                                    <div class="text-center">
                                        <button type="submit" class="btn btn-primary">
                                            Change Password
                                        </button>
                                    </div>
                                </form>
                                <!-- End Change Password Form -->
                            </div>
                        </div>
                        <!-- End Bordered Tabs -->
                    </div>
                </div>
            </div>
        </div>
    </section>
</div>
</template>

<script>
import Auth from '../../Auth'
export default {
    data() {
        return {
            loggedUser: this.auth.user,
            form: {
                name: this.auth.user.name,
                father_name: this.auth.user.father_name,
                gfather_name: this.auth.user.gfather_name,
                company: this.auth.user.company,
                country: this.auth.user.country,
                address: this.auth.user.address,
                phone: this.auth.user.phone,
                email: this.auth.user.email

            },
            pass: {
                current_password: '',
                new_password: '',
                new_password_confirmation: ''
            },
            errors: '',
            passerrors: '',
            currError:''
        }
    },
    methods: {
        updateProfile() {
            let id = this.auth.user.id
            this.axios.patch('/api/user/updateProfile/' + id, this.form)
                .then(() => {
                    // this.$router.push("/home/user-profile");
                    this.axios.post('/api/logout')
                    Auth.logout(); //reset local storage
                    this.$router.push('/login');
                    Toast.fire({
                        icon: "success",
                        title: "Your Profile Updated Successfully ",
                    });
                })
                .catch((error) => {
                    switch (error.response.status) {
                        case 422:
                            this.errors = error.response.data.errors;
                            break;
                        case 401:
                        Toast.fire({
                                icon: "error",
                                title: error.response.data.message,
                            });
                            break;
                        case 403:
                            Toast.fire({
                                icon: "error",
                                title: error.response.data.message,
                            });
                            break;
                        case 500:
                            Toast.fire({
                                icon: "error",
                                title: "Something went wrong please,Please try again!",
                            });
                            break;
                        default:
                            Toast.fire({
                                icon: "error",
                                title: "Some error occurred, Please try again!",
                            });

                            break;
                    }
                });
        },
        passwordUpdate(){
            let id = this.auth.user.id
            this.axios.patch('/api/user/updatePassword/'+id,this.pass)
            .then(() => {
                    // this.$router.push("/home/user-profile");
                    this.axios.post('/api/logout')
                    Auth.logout(); //reset local storage
                    this.$router.push('/login');
                    Toast.fire({
                        icon: "success",
                        title: "Your Password Updated Successfully ",
                    });
                })
                .catch((error) => {
                    switch (error.response.status) {
                        case 422:
                            this.passerrors = error.response.data.errors;
                            break;
                        case 401:
                        this.currError=error.response.data.message
                            this.passerrors=''
                            break;
                        case 403:
                            Toast.fire({
                                icon: "error",
                                title: error.response.data.message,
                            });
                            break;
                        case 500:
                            Toast.fire({
                                icon: "error",
                                title: "Something went wrong please,Please try again!",
                            });
                            break;
                        default:
                            Toast.fire({
                                icon: "error",
                                title: "Some error occurred, Please try again!",
                            });

                            break;
                    }
                });
        } 
    },

};
</script>
